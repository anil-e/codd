...
pub(super) fn initial_filter(columns: &[TableColumn]) -> Option<TableFilter> {
    ...
    Some(TableFilter {
        ...
        operator,
        value: default_filter_value(column, operator),
    })
}
...
fn filter_row(
    ...
    let column_dropdown = gtk::DropDown::builder()
        ...
        .selected(column_index(columns, &filter.column_name) as u32)
        .hexpand(true)
        .build();
    ...
    let operator_dropdown = gtk::DropDown::builder()
        ...
        .selected(operator_index(operators, filter.operator) as u32)
        .hexpand(true)
        .build();
    ...
    if filter.operator.needs_value() {
        fields.attach(
            &value_editor(index, columns, filter, state, sender),
    ...
    column_dropdown.connect_selected_notify({
        ...
        move |dropdown| {
            ...
            if let Some(filter) = updated.get_mut(index) {
                filter.column_name.clone_from(&column.name);
                filter.operator = FilterOperator::for_column(column)[0];
                filter.value = default_filter_value(column, filter.operator);
            }
    ...
    operator_dropdown.connect_selected_notify({
        ...
        move |dropdown| {
            let mut updated = state.borrow().clone();
            let Some(filter) = updated.get_mut(index) else {
                return;
            };
...
fn value_editor(
    ...
    if column.type_group == ColumnTypeGroup::Boolean {
        return choice_editor(index, &["false", "true"], filter, state, sender);
    }
    ...
    if !column.enum_values.is_empty() {
        let values = column
            ...
            .map(String::as_str)
            .collect::<Vec<_>>();
        return choice_editor(index, &values, filter, state, sender);
    }

    let entry = gtk::Entry::builder()
        .text(filter.value.as_deref().unwrap_or_default())
...
fn choice_editor(
    ...
    let dropdown = gtk::DropDown::builder()
        ...
        .selected(choice_index(values, filter.value.as_deref()) as u32)
        .hexpand(true)
        .build();
...
fn normalize_filters(columns: &[TableColumn], filters: &[TableFilter]) -> Vec<TableFilter> {
    filters
        ...
        .filter_map(|filter| {
            ...
            let operator = if filter.operator.is_supported_for(column) {
                filter.operator
            } else {
...
fn column_index(columns: &[TableColumn], column_name: &str) -> usize {
    columns
        .iter()
